...
var LinLib = (function() {
    ...
    // Fraction intern pool
    var fractInterns = {};

    /**
     * Replaces the fraction constructor with interning functionality
     *
     * @requires den != 0
     * @param int num, the numerator of the fraction
     * @param int den, the denominator of the fraction
     */
    Fraction.make = function(num, den) {
        // TODO: Find cleaner solution than copy of toString
        var key = "";
    ...
    //constants
    var ZERO = Fraction.make(0, 1);
    var ONE = Fraction.make(1, 1);
    var NEGATIVE_ONE = Fraction.make(-1, 1);
	...
	var parseFraction = function(s) {
		...
		if(s.indexOf('/') != -1) {
			var parts = s.split('/');
			return Fraction.make(parseInt(parts[0]), parseInt(parts[1]));
		}
		return Fraction.make(parseInt(s), 1);
	}
    ...
    /**
     * Returns a matrix from an array of numbers. Will round each number to an integer.
     * @requires the arr is a well formed rectangularly nested array of numbers
     * @param arr[[number]]
     */
    var matrixFromNumberArray = function(arr) {
        var result = [];
        for(var i = 0; i < arr.length; i++) {
            result.push([]);
            for(var j = 0; j < arr[0].length; j++) {
                result[i].push(Fraction.make(arr[i][j], 1));
            }
        }
        return new Matrix(result);
    };
	...
	function Fraction(num, den) {
		...
		self.add = function(f) {
			...
			if(f.isZero())
				return self;
			return Fraction.make(self.getNum() * f.getDen() + f.getNum() * self.getDen(),
		...
		self.mult = function(f) {
			...
			if(f.isZero())
				return ZERO;
			return Fraction.make(self.getNum() * f.getNum(),
				f.getDen() * self.getDen());
		};
		...
		self.div = function(f) {
			...
			if(self.isZero())
				return ZERO;
			return Fraction.make(self.getNum() * f.getDen(),
		...
		self.invert = function() {
			...
			if(self.isZero())
				return ZERO;
			return Fraction.make(self.getDen(), self.getNum());
		};
		...
		self.negate = function() {
			...
			if(self.isZero())
				return ZERO;
			return Fraction.make(-self.getNum(), self.getDen());
		};
	...
	function Matrix(arr) {
         ...
         *
         * @return Fraction f where f is at i,j
         */
        ...
        /**
         * Applies a function to each matrix element, returning a new matrix
         * @param f the function to apply (takes a Fraction, and returns the new element)
         */
        self.forEachElement = function(f) {
            record('Matrix.toArray', self, [f]);
            result = data.slice(0);
            for(var i = 0; i < result.length; i++) {
                result[i] = result[i].slice(0);
        ...
        /**
         * Convolves a mask over this matrix setting each element to the convolution of the mask over
         * that particular element. If a border element is processed and a part of the convolution is
         * out of the border, these are not taken into account.
         *
         * @param Matrix mask
         */
        self.convolve = function(mask) {
            if (mask.rowCount() > self.rowCount() || mask.colCount() > self.colCount()) {
                throw 'Mask cannot be bigger than matrix';
            }
            var result = [];
            var halfH = Math.floor(mask.rowCount() / 2);
            var halfW = Math.floor(mask.colCount() / 2);

            for (var i = 0; i < self.rowCount(); i++) {
                result.push([]);
                for (var j = 0; j < self.colCount(); j++) {

                    // Apply mask to this element
                    var convolved = ZERO;
                    for (var mi = 0; mi < mask.rowCount(); mi++) {
                        for (var mj = 0; mj < mask.colCount(); mj++) {
                            var translatedRow = i - halfW + mi;
                            var translatedCol = j - halfH + mj;

                            if (translatedRow >= 0 && translatedCol >= 0 &&
                                translatedRow < self.rowCount() && translatedCol < self.colCount()) {
                                convolved = convolved.add(mask.get(mi, mj).mult(self.get(translatedRow, translatedCol)));
                            }
                        }
                    }

                    result[i].push(convolved);
                }
            }
        ...
        /**
         * Normalize the matrix in the following way:
         * 1. Find the max, min
         * 2. Find the the placement of each element within that max/min
         * 3. Compute a percentage of that placement
         * 4. Apply a number n to each percentage
         * 5. Round each result, and make it a fraction
         * 6. Return this matrix
         *
         * @param number n The scalar to multiply the normalization by
         */
        self.normalize = function(n) {
            ...
            for (var i = 0; i < result.length; i++) {
                for (var j = 0; j < result[0].length; j++) {
                    var dec = result[i][j].toDec();
                    dec -= min;
                    var percentage = dec / dist;
                    var newValue = Math.round(n * percentage);

                    result[i][j] = LinLib.Fraction.make(newValue, 1);
                }
            }
	...
	return {
            ...
            'matrixFromNumberArray' : matrixFromNumberArray,
